...
### Put the DHE assembly in the App
...
```csharp title="Copy DHE assembly during building process"
...
- Make sure you have run `HybridCLR/CreateAOTDllSnapshot` to back up the AOT file, and make sure that the AOT dll in the snapshot directory is the AOT dll generated during building.
- Use `HybridCLR/generate/DHEAssemblyOptionDatas` to generate dhao files.

:::caution
Because the core work of DHEAssemblyOptionDatas is to compare the latest hot update `DHE dll` with original AOT dll those were saved in the snapshot directory, and generate changed function and type information. Please ensure The correctness of the hot update dll and the backuped AOT dll!
:::
